...
#### [1] proj_group ~ ai_group*prior_group + (1|content) ----
...
# make tmp dataset for "know"
tmp = d %>%
  filter(short_trigger == "know") %>%
  group_by(content) %>% mutate(projMean=mean(projective), aiMean=mean(ai)) %>%
  group_by(content,prior_type) %>% mutate(priorMean=mean(prior)) %>%
  select(c(short_trigger,projMean,aiMean,priorMean,content)) %>%
  distinct(projMean,aiMean,priorMean,.keep_all = TRUE)

nrow(tmp) #40

# center predictors
tmp$caiMean = tmp$aiMean-mean(tmp$aiMean)
tmp$cpriorMean = tmp$priorMean-mean(tmp$priorMean)
#view(tmp)

# check the variables
summary(tmp$projMean) #doesn't need to be transformed for beta regression
summary(tmp$caiMean)
summary(tmp$cpriorMean)

# proj_group ~ ai_group*prior_group + (1|content) 
betamodel = bf(projMean ~ caiMean*cpriorMean + (1|content),
               phi ~ caiMean*cpriorMean + (1|content), # beta distribution's precision 
               family = Beta())

m.tmp = brm(formula = betamodel,
            family=Beta(),
            data=tmp, 
...
m.tmp1a = brm(formula = betamodel,
            family=Beta(),
            data=tmp, 
            cores = 4, iter = 5000, warmup = 2000,
            control = list(adapt_delta = .95,max_treedepth=15))
summary(m.tmp1a)
...
tmp = d %>%
  ...
  group_by(content) %>% mutate(aiMean=mean(ai)) %>%
  select(c(short_trigger,projective,aiMean,prior,content,workerid)) #%>%
  #distinct(projective,aiMean,prior,.keep_all = TRUE)

nrow(tmp) #505

# center predictors
tmp$caiMean = tmp$aiMean-mean(tmp$aiMean)
tmp$cprior = tmp$prior-mean(tmp$prior)
#view(tmp)

# check the variables
summary(tmp$projective) #needs to be transformed for beta regression
summary(tmp$caiMean)
summary(tmp$cprior)

# create betaProjective
nRow = 505
nRowMinusOne = nRow-1
tmp$betaProjective = (tmp$projective*nRowMinusOne + .5)/nRow
summary(tmp$betaProjective)
...
#### [1] projMean ~ caiMean*cpriorMean + (1|content) ----
betamodel = bf(projMean ~ caiMean*cpriorMean + (1|content),
               phi ~ caiMean*cpriorMean + (1|content), # beta distribution's precision 
               family = Beta())
...
for (p in predicates) {
  ...
  saveRDS(assign(paste("model.", p, ".rds", sep=""), brm(formula = betamodel,
                                                         ...
                                                         cores = 4, iter = 3000, warmup = 500,
                                                         control = list(adapt_delta = .95,max_treedepth=15))),
          file=paste("models/RQIIIa/1-mean-ai-mean-prior/model.",p, ".rds", sep=""))
}

tmp <- readRDS("models/RQIIIa/1-mean-ai-mean-prior/model.acknowledge.rds")
summary(tmp)
...
# create a dataset for each predicate with the relevant variables
...
# create group-level projection (mean projection for each predicate/content)
d <- d %>% 
  group_by(short_trigger,content) %>% 
  mutate(projMean = mean(projective))

# create group-level at-issueness (mean at-issueness for each predicate/content)
d <- d %>% 
  group_by(short_trigger,content) %>% 
  mutate(aiMean = mean(ai))
table(d$aiMean)
...
# create group-level prior beliefs (mean prior for each content/fact)
d <- d %>% 
  group_by(content,prior_type) %>% 
  mutate(priorMean = mean(prior))
table(d$priorMean)
#view(d)
...
# does aiMean need to be transformed?
min(d$aiMean) #0.1204762
max(d$aiMean) #0.9292
# no, because aiMean is already in (0,1)

# does projMean need to be transformed?
min(d$projMean) #0.1031579
max(d$projMean) #0.9338235
# no, because projMean is already in (0,1)
...
for (p in predicates) {
  ...
  write_csv(assign(paste("data.", p, sep=""), d %>% filter(short_trigger == p) %>% 
                                                    ...
                                                    #mutate(cpriorMean = priorMean-mean(priorMean), caiMean = aiMean-mean(aiMean)) %>%
                                                    mutate(betaProjective = (projective*nRowMinusOne + .5)/nRow) %>%
...
# new loop to add cpriorMean and caiMean because it doesn't work above for some unexplainable reason
for (p in predicates) {
  data = read_csv(paste("models/data.", p, ".csv",sep=""))
  write_csv(assign(paste("data.", p, sep=""), data %>% mutate(cpriorMean = priorMean-mean(priorMean), caiMean = aiMean-mean(aiMean))),
            file=paste("models/data.",p,".csv",sep=""))
}

# check the variables
tmp = read_csv("models/data.acknowledge.csv")
summary(tmp$cprior)
summary(tmp$cai)
summary(tmp$cpriorMean) 
summary(tmp$caiMean)
...
betamodel = bf(betaAiMean ~ cprior + (1+cprior|content),
...
for (p in predicates) {
  ...
  saveRDS(assign(paste("model.", p, ".rds", sep=""), brm(formula = betamodel,
                                                         ...
                                                         cores = 4, iter = 3000, warmup = 500,
                                                         control = list(adapt_delta = .95,max_treedepth=15))),
          file=paste("models/1-group-level-ai/RQIIIb/model.",p, ".rds", sep=""))
}
