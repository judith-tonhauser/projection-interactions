# script to compare the participants of Exps 1 and 2 of proj/ai/prior paper
# Exp 1 in paper is Exp 3, Exp 2 in paper is Exp 2
...
# read in the data
exp1 = read_csv("exp3/experiment-workerids.csv") 
nrow(exp1) #600 participants 
length(unique(exp1$workerid)) #600

exp2 = read_csv("exp2/experiment-workerids.csv") 
nrow(exp2) #600 participants 
length(unique(exp2$workerid)) #600
...
d <- droplevels(subset(d, !d$workerid %in% lvw$workerid))
length(unique(d$workerid)) #566 Turkers remain (7 excluded)
...
d.MC <- d %>%
  ...
  droplevels()
nrow(d.MC) #6792 / 565 Turkers = 12 (6 main clause controls in projection and ai blocks)

# projection of main clause data
d.MC.Proj <- d.MC %>%
  filter(question_type == "projective") %>%
  droplevels()
nrow(d.MC.Proj) #3396 / 566 Turkers = 6 main clause controls in projection block
...
nrow(outliers) #708 / 12 = 59 Turkers
...
d <- d %>%
  filter(!(workerid %in% p$workerid | workerid %in% ai$workerid)) %>% 
  droplevels()
length(unique(d$workerid)) #507 Turkers (59 Turkers excluded)

# age info for all remaining Turkers
summary(d$age) #19-73, mean: 38
table(d$age)
ggplot(d, aes(x=age)) +
  geom_histogram()

# number of data points
nrow(d[d$short_trigger != "MC",]) #20280
...
# number of data points
nrow(d[d$short_trigger != "MC",]) #9980
...
om_exp1 <- read_csv("https://raw.githubusercontent.com/judith-tonhauser/attitude_preds_projection/master/results/meta-analysis/data/om_exp1.csv")
nrow(om_exp1) #5720

om_exp2a <- read_csv("https://raw.githubusercontent.com/judith-tonhauser/attitude_preds_projection/master/results/meta-analysis/data/om_exp2a.csv")
nrow(om_exp2a) #1500
