# script to remove data from participants who took the experiment more than once
# this works on workerids so do not move this to a directory shared in the repo
...
# read in the file with the workerids
d = read_csv("../experiment-trials.csv")
summary(d)

table(d$workerid)

# read in the file with the workerids
subjects = read_csv("../experiment-subject_information.csv")
summary(subjects)

length(subjects$workerid) #300
length(unique(subjects$workerid)) #289

# make the submit time readable by R
d$time <- d$assignmentsubmittime
d$time <- gsub("Wed Mar 20 ","",d$time)
d$time <- gsub("Thu Mar 21 ","",d$time)
d$time <- gsub(" PDT 2019","",d$time)
d$time <- as.POSIXct(d$time,format="%H:%M:%S")
head(d$time)

# read in the file with the workerids
ids = read_csv("../experiment-workerids.csv")

length(ids$workerid) #289
length(unique(ids$workerid)) #289
